...
@Component({
  ...
  templateUrl: './modellogout.component.html',
  styleUrl: './modellogout.component.css'
})
export class ModellogoutComponent implements OnInit{
        ...
        this.actividadService.crearActividad(actividadData).subscribe({
          next: (resp) => {
            ...
            this.fileService.updateFileCategory(this.data.id, "Reemplazar").subscribe({
              next: (response) =>
                console.log('File edited successfully', response, this.mostrarMensajeRegistroExito(), this.router.navigate(['/cloud/home'], { queryParams: { number: 1 } })),
