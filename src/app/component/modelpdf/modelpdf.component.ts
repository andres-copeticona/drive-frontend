...
export class ModelpdfComponent implements OnInit {
    ...
    this.filteredOptions = this.myControl.valueChanges.pipe(
      ...
      map((value) => {
        ...
        this.selectedFolderId =
          typeof value === 'string' ? value : value?.usuarioID.toString() || '';
        return name ? this._filter(name as string) : this.options.slice();
      }),
    );
  }
  gotoreemplazar() {
    this.router.navigate(['/cloud/reemplazar'], {
      queryParams: {
        number: this.data.id,
      ...
      next: (ipResponse) => {
        ...
        this.actividadService.crearActividad(actividadData).subscribe({
          next: (resp) => {
            ...
            this.fileService
              .updateFileCategory(this.data.id, 'Reemplazado')
              .subscribe({
                next: (response) =>
                  console.log(
  ...
  async modifyPdf() {
    ...
    const qrCodeDataUri = QRCode.toDataURL(
      this.baseurl + '/detalleSello?signed=' + this.auxResponse || 'Firmado',
    );
    ...
    const file = new File([blob], 'Sellado_' + this.data.name, {
      type: 'application/pdf',
    });
      ...
      next: (ipResponse) => {
        ...
        this.actividadService.crearActividad(actividadData).subscribe({
          next: (resp) => {
            ...
            // Usar fileService para subir el archivo
            this.fileService
              .uploadFile(this.data.folderId, fileToUpload, data)
              .subscribe({
                next: (response) =>
                  console.log(
